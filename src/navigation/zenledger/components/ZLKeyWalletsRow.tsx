...
import ZLWalletRow from './ZLWalletRow';
...
interface ZKeyWalletProps {
  keys: ZLKey[];
  onPress: (keyId: string, wallet?: ZLWallet) => void;
  onDropdownPress: (keyId: string) => void;
}

const ZLKeyWalletsRow = ({keys, onPress, onDropdownPress}: ZKeyWalletProps) => {
    ...
    <View style={{marginBottom: 70}}>
      {keys ? (
        <FlatList
          data={keys}
          keyExtractor={(_item, index) => index.toString()}
          renderItem={({item, index}: {item: ZLKey; index: number}) => {
...
export default ZLKeyWalletsRow;
